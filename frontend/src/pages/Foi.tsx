...
export default function Foi() {
  const [userData, setUserData] = useState<UserData | null>(null);
  const location = useLocation();
  const [activeTab, setActiveTab] = useState<'reflechissons' | 'communaute' | 'livres' | 'zaka' | 'formation-religieux'>('reflechissons');
  const [zakaSubTab, setZakaSubTab] = useState<'pauvres' | 'dons' | 'zakat'>('pauvres');
  ...
  // Charger les données de formation-religieux quand l'onglet est actif
  useEffect(() => {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Chargement de la foi...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex space-x-8">
            {[
              ...
              { id: 'zaka', label: 'Zaka et Dons', icon: '🤲' }
            ].map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as any)}
                ...
                activeTab === tab.id
                    ? tab.id === 'zaka' ? 'border-green-500 text-green-600' : 'border-blue-500 text-blue-600'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {activeTab === 'communaute' && (
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold text-gray-900">👥 Communautés Religieuses</h2>
              <button
        ...
        {activeTab === 'zaka' && (
          <div className="space-y-6">
            {/* Navigation sous-onglets pour Zaka */}
            <div className="bg-white border-b">
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <nav className="flex space-x-8">
                  {[
                    ...
                    <button
                      key={tab.id}
                      onClick={() => setZakaSubTab(tab.id as any)}
                      className={`py-4 px-1 border-b-2 font-medium text-sm ${
                        zakaSubTab === tab.id
            ...
            {/* Contenu Liste des Pauvres */}
            {zakaSubTab === 'pauvres' && (
            <div className="space-y-6">
              {/* Message informatif pour les musulmans */}
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-blue-700">
                      <strong>Note importante :</strong> La <strong>Zakat</strong> (aumône obligatoire) est destinée aux pauvres musulmans uniquement. La <strong>Sadaqah</strong> (aumône facultative) peut être donnée à tous les pauvres, quelle que soit leur religion. Utilisez le filtre "Type de don" pour voir les personnes éligibles.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                <div className="flex flex-col md:flex-row gap-4">
                  ...
                  <select
                    ...
                    onChange={(e) => setDonationTypeFilter(e.target.value as 'zakat' | 'sadaqah' | 'all')}
                    className="border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-green-500 bg-green-50"
                    title="Filtre par type de don"
                  >
                    <option value="all">Tous les types de dons</option>
                    <option value="zakat">Zakat (Musulmans uniquement)</option>
                    <option value="sadaqah">Sadaqah (Tous les pauvres)</option>
                  </select>
                </div>
              </div>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {stages.length > 0 ? stages.map((stage) => (
                  <div key={stage.id} className="border rounded-lg p-6 hover:shadow-md transition-shadow">
